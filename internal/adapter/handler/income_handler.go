...
func (h *IncomeHandler) Create(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		...
		response.ErrorWithLog(w, r, http.StatusInternalServerError, "failed to create income", err.Error())
		return
	}
...
func (h *IncomeHandler) Get(w http.ResponseWriter, r *http.Request) {
	...
	if strings.TrimSpace(uuid) == "" {
		response.ErrorWithLog(w, r, http.StatusBadRequest, "invalid income uuid", nil)
		return
	}
	...
	if err != nil {
		...
		response.ErrorWithLog(w, r, http.StatusInternalServerError, "failed to get income", err.Error())
		return
	}
...
func (h *IncomeHandler) Update(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		...
		response.ErrorWithLog(w, r, http.StatusInternalServerError, "failed to update income", err.Error())
		return
	}
...
func (h *IncomeHandler) Delete(w http.ResponseWriter, r *http.Request) {
	...
	if err := h.svc.DeleteIncome(r.Context(), userID, uuid); err != nil {
		...
		response.ErrorWithLog(w, r, http.StatusInternalServerError, "failed to delete income", err.Error())
		return
	}
